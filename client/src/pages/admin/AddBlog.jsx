...
const AddBlog = () => {
  ...
  const generatecontent = async () => {
    ...
    try {
      ...
      if(data.success){
        quillRef.current.roor.innerHTML = parse(data.content)
      }
      else{
  ...
  return (
    <form className='flex-2 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
        ...
        <select onChange={e => setCategory(e.target.value)} name="category"
          className='mt-2 px-3 py-2 border text-gray-500 border-gray-300
        ...
        <div className='flex gap-2 mt-4'>
          ...
          <button onClick={onSubmitHandler} disabled={isAdding} type='submit' className='mt-8 w-40 h-10 bg-primary
